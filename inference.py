from dynamic_masking import create_dynamic_mask, apply_mask
        ...
        for p, f, c in zip(pred, frames, coords):
            ...
            if args.quality in ["Enhanced", "Improved"]:
                if str(args.mouth_tracking) == "True":
                    # p, last_mask = create_dynamic_mask(cf)   Here the code updated for dynamic masking (@harshsaini) 727-731
                    mask = create_dynamic_mask(cf)
                    if mask is not None:
                        p = apply_mask(cf, p, mask)
